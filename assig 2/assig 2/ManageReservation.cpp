...
ManageReservation::ManageReservation(DatabaseHelper* dbHelper)
{
	pagePath = "/manage-reservation";
	...
	string htmlFile = fileOperations.getTextfromFile("reservation.html");

	//###Seats###
	//Find location to split
	string splitPoint = "<div class=\"orderItem orderItemTitle\"><b>Seats</b></div>";
	...
	//###time and date###
	//Find location to split
	splitPoint = "<div class=\"orderItem orderItemTitle\"><b>Time and Date</b></div>";
	...
	//###customer name###
	//Find location to split
	splitPoint = "<div class=\"orderItem orderItemTitle\"><b>Customer name</b></div>";
	splitLocation = htmlFile.find(splitPoint) + splitPoint.size();
	//Split into the two string
...
string ManageReservation::runPage(PageRequest request)
{
	//Prepare regex match
...
string ManageReservation::generateList()
{
	//Get list of items
	vector<vector<string>>* results = db->runQuery("SELECT `ID`, `Seats`, `Reservation_timestamp`,`Customer_ID` FROM `Reservation`");
	...
	const string listItemStart = "<div onclick=\"post('/manage-reservation',{flag : 'form', tableID : '";
	...
	{//Items in list
	 //Loop through items
		for (auto i = results->begin(); i != results->end(); ++i)
		{
			...
			{
			case 4:
				statusColum += listItemStart + i->at(0) + listItemStart2 + i->at(3) + listItemEnd;
			case 3:
				seatsColum += listItemStart + i->at(0) + listItemStart2 + i->at(2) + listItemEnd;
			case 2:
				tableNumberColum += listItemStart + i->at(0) + listItemStart2 + i->at(1) + listItemEnd;
			}
		}
...
string ManageReservation::prepareForm(string id)
{
	string seats = "1";
	string hiddenField;
	string hiddenField2;
...
bool ManageReservation::updateRecord(PageRequest request)
{
...
void ManageReservation::removeRecord(string id)
{
	//Prepare parameters
	vector<BindParam> params;
	params.push_back(BindParam(intType, id));
